...
import {addLocalDays,buildDailyPlan,classifyQuestion,dueIds,fcKey,localDateKey,queueBuckets,speedStats,subjectStats,updateSrs} from './learning-core.js?v=9';
const $=s=>document.querySelector(s), $$=s=>[...document.querySelectorAll(s)];
const STORAGE_KEY='fc-quiz-state-v2',SESSION_KEY='fc-quiz-session-v2',THEME_KEY='fc-quiz-theme';
const ASSET_BASE='./';
// 概念卡：只有答完題要看詳解時才用得到，所以不進開機的必要路徑（questions.json 已經 7 MB）。
// 失敗就退回 {}，讓 renderConceptCards 靜靜不渲染——概念卡載不到不該讓整題看不了詳解。
// 詳解裡有 95 處寫成 **粗體** 的 markdown（各科作者用來標重點），
// 但渲染是純 escapeHtml，線上會原樣顯示成星號。先跳脫再把 ** 轉成 <strong>：
// 順序很重要——escapeHtml 在前，所以資料裡的角括號永遠進不了 HTML。
function mdBold(s){return escapeHtml(s).replace(/\*\*([^*]+)\*\*/g,'<strong>$1</strong>')}
let conceptData=null,conceptPending=null;
function loadConcepts(){if(conceptData)return Promise.resolve(conceptData);
 if(!conceptPending)conceptPending=fetch(`${ASSET_BASE}concepts.json`).then(r=>r.ok?r.json():{}).catch(()=>({})).then(j=>{conceptData=j;return j});
...
function defaultState(){return{attempts:[],wrong:{},exams:[],conquered:{},flags:{},notes:{},noteLevels:{},srs:{},examDate:'',dailyCompletions:{},theme:'room'}}
...
$('#flagButton').onclick=toggleFlag;$('#noteButton').onclick=openNote;$('#saveNote').onclick=saveNote;$$('[data-level]').forEach(b=>b.onclick=()=>setNoteLevel(b.dataset.level));$('#reviewButton').onclick=startReview;
...
function renderFeedback(q,reveal,selected){const el=$('#feedback');if(!reveal){el.classList.add('hidden');return}const correct=isCorrect(q,selected),exp=q.explanation;let details='<p>詳解撰寫中</p>';if(exp&&typeof exp==='object'){const summary=exp.summary?`<div class="summary"><span class="lbl">考點</span><br>${mdBold(exp.summary)}</div>`:'',list=`<ul class="explanation-list">${Object.keys(q.options).map(k=>`<li class="${q.answer.includes(k)?'key':''}"><b>${k}.</b> ${mdBold(exp[k]||'詳解撰寫中')}</li>`).join('')}</ul>`,guideline=exp.guideline?`<p class="guideline"><span class="lbl">依據</span>　${escapeHtml(exp.guideline)}</p>`:'';details=summary+list+guideline}const ref=q.fc_ref?formatFcRef(q.fc_ref):'',note=state.notes[q.id]?`<p class="note-copy lv-${noteLevel(q.id)}"><b>我的註記 · ${NOTE_LEVELS.find(l=>l[0]===noteLevel(q.id))[1]}</b><br>${escapeHtml(state.notes[q.id])}</p>`:'';el.innerHTML=`<h2 class="${correct?'correct-copy':'wrong-copy'}">${correct?'答對了':'這題答錯了'}</h2><p>正確答案：<b>${q.answer.join('／')}</b></p>${q.answer_note?`<p><b>官方更正：</b>${escapeHtml(q.answer_note)}</p>`:''}${details}${renderConceptCards(q)}${ref}${note}${renderConceptLinks(q)}`;el.classList.remove('hidden')
 // 概念卡還沒載到就先渲染其他部分，載完再重畫一次。載失敗時 conceptData 會是 {}，不會無限重試。
 if(!conceptData&&exp&&typeof exp==='object'&&Array.isArray(exp.concepts)&&exp.concepts.length)loadConcepts().then(()=>renderFeedback(q,reveal,selected))}
// 概念卡：資料早就有（263 張、500 題掛卡），2026-09-04 前一直沒有介面出口。
function renderConceptCards(q){const exp=q.explanation;if(!exp||typeof exp!=='object')return'';
 const ids=Array.isArray(exp.concepts)?exp.concepts:[];if(!ids.length||!conceptData)return'';
...
// 註記分三級。存成獨立的 noteLevels，不動 notes 的字串格式——舊資料才不會壞。
const NOTE_LEVELS=[['red','要命','完全不會，考前一定要回來'],['yellow','沒把握','記得但不確定'],['green','小提醒','只是補一句備忘']];
const noteLevel=id=>state.noteLevels?.[id]||'yellow';
function setNoteLevel(v){$$('[data-level]').forEach(b=>b.setAttribute('aria-pressed',String(b.dataset.level===v)))}
function currentNoteLevel(){return $$('[data-level]').find(b=>b.getAttribute('aria-pressed')==='true')?.dataset.level||'yellow'}
function openNote(){const q=currentQuestion();$('#noteTitle').textContent=paperLabel(q);$('#noteText').value=state.notes[q.id]||'';setNoteLevel(noteLevel(q.id));$('#noteDialog').showModal()}
function saveNote(){const id=currentQuestion().id,text=$('#noteText').value.trim();
 if(!state.noteLevels)state.noteLevels={};
 if(text){state.notes[id]=text;state.noteLevels[id]=currentNoteLevel()}else{delete state.notes[id];delete state.noteLevels[id]}
 saveState();$('#noteDialog').close();renderQuestion();toast('註記已儲存')}
...
function renderAnalysis(){const attempts=state.attempts,correct=attempts.filter(a=>a.correct).length,rate=attempts.length?Math.round(correct/attempts.length*100):0,speeds=speedStats(attempts,new Map(questions.map(q=>[q.id,q]))),subjects=subjectStats(attempts),speedRows=speeds.map(x=>`<div class="lrow ${x.slowButAccurate?'needs-speed':''}"><span>${x.label}${x.slowButAccurate?' · 要練速度':''}</span><em class="mono">平均 ${x.average}s　正確率 ${x.rate}%　${x.n} 題</em></div>`).join(''),subjectRows=subjects.length?subjects.map(x=>`<div class="lrow"><span>${escapeHtml(x.name)}</span><em class="mono">平均 ${x.average}s　正確率 ${x.rate}%</em></div>`).join(''):'<p class="tiny">完成題目後顯示。</p>';$('#analysisContent').innerHTML=`<div class="heat"><div class="metric"><b>${attempts.length}</b><span>累積作答</span></div><div class="metric"><b>${rate}%</b><span>累積正確率</span></div><div class="metric"><b>${attempts.length?Math.round(attempts.reduce((s,a)=>s+(a.seconds||0),0)/attempts.length):0}s</b><span>平均每題</span></div></div><div class="analysis-card"><span class="lbl">作答速度 · 國考目標每題 90 秒</span><div class="link">${speedRows}</div><p class="tiny">有附圖＝影像；無圖且題幹 ≥ 120 字＝長題幹；其餘＝一般。正確率 ≥ 60% 且平均超過 90 秒會標示要練速度。</p></div><div class="analysis-card"><span class="lbl">單科平均作答時間 · 最弱的先出現</span><div class="link">${subjectRows}</div></div><div class="analysis-card" id="weakCards"><span class="lbl">最弱的概念卡 · 載入中</span></div>${renderExamTrend()}${renderNoteList()}`;showView('analysisView');renderWeakCards();
 $$('[data-note-id]').forEach(b=>b.onclick=()=>{const q=questions.find(x=>x.id===b.dataset.noteId);
  if(q)startSession([q],'note','我的註記',false)})}
// 註記本來只在答完那一題時才看得到，考前想「把我標過的東西整個看一遍」沒有地方去。
// 依重要程度排（要命→沒把握→小提醒），點一則就直接開那題。
function renderNoteList(){const ids=Object.keys(state.notes||{}).filter(id=>String(state.notes[id]).trim());
 if(!ids.length)return`<div class="analysis-card"><span class="lbl">我的註記</span><p class="tiny">還沒有註記。作答時按「✎ 加註記」寫下易混淆處，這裡會集中列出。</p></div>`;
 const order={red:0,yellow:1,green:2},byId=new Map(questions.map(q=>[q.id,q]));
 const rows=ids.map(id=>({id,q:byId.get(id),lv:noteLevel(id),text:String(state.notes[id])}))
  .sort((a,b)=>order[a.lv]-order[b.lv]||String(a.id).localeCompare(String(b.id)));
 const count=l=>rows.filter(r=>r.lv===l).length;
 return`<div class="analysis-card"><span class="lbl">我的註記 · 要命 ${count('red')}　沒把握 ${count('yellow')}　小提醒 ${count('green')}</span><div class="link">${
  rows.map(r=>`<button class="lrow note-row" data-note-id="${escapeHtml(r.id)}"><span><b class="ndot lv-${r.lv}"></b>${escapeHtml(r.q?paperLabel(r.q):r.id)}　${escapeHtml((r.q?.stem||'').slice(0,26))}<small class="hitsnip">${escapeHtml(r.text.slice(0,70))}${r.text.length>70?'…':''}</small></span><em class="mono">${NOTE_LEVELS.find(l=>l[0]===r.lv)?.[1]||''}</em></button>`).join('')
 }</div></div>`}
// 模擬考成績趨勢：state.exams 只收 mode==='mock' 的整卷，本來只被模擬考列表拿去顯示
// 「上次幾分」，沒有地方看得出來到底有沒有進步——那是備考時最想知道的一件事。
function renderExamTrend(){const ex=[...(state.exams||[])].sort((a,b)=>String(a.date).localeCompare(String(b.date)));
 if(!ex.length)return`<div class="analysis-card"><span class="lbl">模擬考成績</span><p class="tiny">還沒有整卷模擬考紀錄。到「模擬考」做完一整卷並交卷後，這裡會畫出趨勢。</p></div>`;
 const avg=Math.round(ex.reduce((s,e)=>s+e.score,0)/ex.length),last=ex.at(-1),
  best=Math.max(...ex.map(e=>e.score)),
  bars=ex.slice(-16).map(e=>`<div class="ebar" title="${escapeHtml(e.title)}　${e.score}%"><b style="height:${Math.max(2,e.score)}%" class="${e.score>=60?'pass':'fail'}"></b><small>${e.score}</small></div>`).join(''),
  rows=[...ex].reverse().slice(0,8).map(e=>`<div class="lrow"><span>${escapeHtml(String(e.date).slice(0,10))}　${escapeHtml(e.title||'')}</span><em class="mono">${e.score}%　${e.correct}／${e.total}　${Math.round((e.seconds||0)/60)} 分鐘</em></div>`).join('');
 return`<div class="analysis-card"><span class="lbl">模擬考成績 · 60 分是及格線</span><div class="heat"><div class="metric"><b>${last.score}%</b><span>最近一次</span></div><div class="metric"><b>${avg}%</b><span>平均</span></div><div class="metric"><b>${best}%</b><span>最佳</span></div></div><div class="ebars">${bars}</div><div class="link">${rows}</div></div>`}

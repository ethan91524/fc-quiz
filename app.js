...
function renderFeedback(q,reveal,selected){const el=$('#feedback');if(!reveal){el.classList.add('hidden');return}const correct=isCorrect(q,selected),exp=q.explanation;let details='<p>詳解撰寫中</p>';if(exp&&typeof exp==='object'){const summary=exp.summary?`<div class="summary"><span class="lbl">考點</span><br>${escapeHtml(exp.summary)}</div>`:'',list=`<ul class="explanation-list">${Object.keys(q.options).map(k=>`<li class="${q.answer.includes(k)?'key':''}"><b>${k}.</b> ${escapeHtml(exp[k]||'詳解撰寫中')}</li>`).join('')}</ul>`,guideline=exp.guideline?`<p class="guideline"><span class="lbl">依據</span>　${escapeHtml(exp.guideline)}</p>`:'';details=summary+list+guideline}const ref=q.fc_ref?formatFcRef(q.fc_ref):'',note=state.notes[q.id]?`<p class="note-copy lv-${noteLevel(q.id)}"><b>我的註記 · ${NOTE_LEVELS.find(l=>l[0]===noteLevel(q.id))[1]}</b><br>${escapeHtml(state.notes[q.id])}</p>`:'';el.innerHTML=`<h2 class="${correct?'correct-copy':'wrong-copy'}">${correct?'答對了':'這題答錯了'}</h2><p>正確答案：<b>${q.answer.join('／')}</b></p>${q.answer_note?`<p><b>官方更正：</b>${escapeHtml(q.answer_note)}</p>`:''}${details}${renderConceptCards(q)}${ref}${note}${renderConceptLinks(q)}`;el.classList.remove('hidden')
